...
namespace ProducerConsumer
{
    ...
    internal sealed class ConcurrentLinkedQueue<T> : IReadOnlyCollection<T>
    {
        ...
        public void Enqueue(T item)
        {
            ...
            while(!newNodeWasAdded)
            {
                ...
                if(_tail == oldTail)
                {
                    if(oldTailNext == null)
                    {
                        newNodeWasAdded = CompareAndSwap(ref _tail.Next, null, newNode);
                    }
                    else
                    {
                        CompareAndSwap(ref _tail, oldTail, oldTailNext);
                    }
                }
            }

            CompareAndSwap(ref _tail, oldTail, newNode);
        }
            ...
            while(!haveAdvancedHead)
            {
                ...
                if(oldHead == _head)
                {
                    if(oldHead == oldTail)
                    {
                        if(oldHeadNext == null)
                            return false;
                        CompareAndSwap(ref _tail, oldTail, oldHeadNext);
                    }
                    else
                    {
                        item = oldHeadNext.Item;
                        haveAdvancedHead = CompareAndSwap(ref _head, oldHead, oldHeadNext);
                    }
                }
            }
            return true;
                ...
                if (h == _head)
                {
                    if (h == t)
                    {
                        if (first == null)
                            ...
                            CompareAndSwap(ref _tail, t, first);
                    }
                    else
                    {
                        if (first.Item != null)
                            ...
                            CompareAndSwap(ref _head, h, first);
                    }
                }
            }
        }
        ...
        private static bool CompareAndSwap<TValue>(ref TValue target, TValue oldValue, TValue newValue)
            where TValue : class =>
                Interlocked.CompareExchange(ref target, oldValue, newValue) == oldValue;
