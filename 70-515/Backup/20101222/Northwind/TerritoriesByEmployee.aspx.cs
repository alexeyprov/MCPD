...
public partial class Northwind_TerritoriesByEmployee : Page
{
	...
	private IAsyncResult Page_BeginDbAsyncCall(object sender, EventArgs e, AsyncCallback cb, object state)
	{
		...
		CompletedSyncResult syncResult = null;
		try
		{
			connection.Open();
		}
		...
		{
			syncResult = new CompletedSyncResult(ix, state, cb);
		}
		catch (SqlException ex)
		{
			syncResult = new CompletedSyncResult(ex, state, cb);
		}
	...
	{
		if (result is CompletedSyncResult)
		{
