...
public partial class Northwind_ProductsByCategory : Page
{
		...
		get
		{
			...
			if (null == retval)
			{
				using (DataContainer data = PrepareDataSet())
				{
					...
					Cache.Insert(PRODUCTS_CATEGORIES_SESSION_KEY, retval, data.Dependency,
	...
	{
		DataContainer data = new DataContainer(WebConfigurationManager.ConnectionStrings[ConstantsHelper.NORTHWIND_ASYNC_CONNECTION_STRING].ConnectionString);
	...
	private class DataContainer : IDisposable
	{
		...
		public void AppendTable(string tableName, string[] columnNames, params CalculatedColumnMetadata[] calculatedCols)
		{
			...
			{
				cmd.CommandText = String.Format(SELECT_FORMAT, String.Join(", ", columnNames), tableName);
