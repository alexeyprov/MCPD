...
{
	public partial class EmployeesPage : Northwind.UI.NorthwindBasePage
	{
		#region Constants
		...
		protected void grdEmployees_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (grdEmployees.SelectedIndex >= 0)
			{
				...
				{
					//never happens, SelectedIndexChanged is always fired before DataBound
				...
				for (int i = 0; i < e.NewValues.Count; ++i)
				{
					...
					if (cell.Controls.Count > 0)
					{
						...
						if (control != null)
						{
							...
							if (control.Text != newValue)
							{
								control.Text = newValue;
								control.ForeColor = Color.Red;
								BoundField field = grdEmployees.Columns[i + 1] as BoundField;
								if (field != null)
								{
									HighlightFieldInConflictingDataView(field.DataField);
								}
							}
						}
					}
		...
		protected void dataSource_ContextCreating(object sender, LinqDataSourceContextEventArgs e)
		{
		...
		protected void srcOrders_Updating(object sender, SqlDataSourceCommandEventArgs e)
		{
			e.Cancel = !IsValid;
		}
		...
		private void HighlightFieldInConflictingDataView(string fieldName)
		{
			var match = from DataControlField field in dvwConflictingEmployee.Fields
						where (field is BoundField) && (0 == String.Compare(((BoundField)field).DataField, fieldName, StringComparison.InvariantCultureIgnoreCase))
						select field.ItemStyle;

			if (match.Count() > 0)
			{
				match.First().ForeColor = Color.Red;
			}
		}
