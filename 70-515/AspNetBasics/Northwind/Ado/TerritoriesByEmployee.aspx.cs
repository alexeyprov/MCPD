...
{
	public partial class TerritoriesByEmployeePage : Northwind.UI.NorthwindBasePage
	{
		#region Constants
		...
		private IAsyncResult Page_BeginDbAsyncCall(object sender, EventArgs e, AsyncCallback cb, object state)
		{
			...
			CompletedSyncResult<int> syncResult = null;
			try
			{
				connection.Open();
			}
			...
			{
				syncResult = new CompletedSyncResult<int>(ix, state, cb);
			}
			catch (SqlException ex)
			{
				syncResult = new CompletedSyncResult<int>(ex, state, cb);
			}
		...
		{
			if (result is CompletedSyncResult<int>)
			{
