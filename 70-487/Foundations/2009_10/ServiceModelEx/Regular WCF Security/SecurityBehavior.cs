...
namespace ServiceModelEx
{
   class SecurityBehavior : IServiceBehavior
   {
      ...
      public void Validate(ServiceDescription description,ServiceHostBase serviceHostBase)
      {
         if(m_SubjectName != null)
         {
            switch(m_Mode)
            {
               ...
               case ServiceSecurity.Relay:
               {
                  ...
                  {
                     subjectName = ServiceBusHelper.ExtractSolution(description.Endpoints[0].Address.Uri);
                  }
                  serviceHostBase.Credentials.ServiceCertificate.SetCertificate(m_StoreLocation,m_StoreName,m_FindType,subjectName);
                  break;
               }
         ...
         else
         {
            switch(m_Mode)
            {
               ...
               case ServiceSecurity.Relay:
               {
                  string subjectName = ServiceBusHelper.ExtractSolution(description.Endpoints[0].Address.Uri);
                  serviceHostBase.Credentials.ServiceCertificate.SetCertificate(m_StoreLocation,m_StoreName,m_FindType,subjectName);
                  break;
               }
            }
      ...
      internal static void ConfigureRelay(Collection<ServiceEndpoint> endpoints)
      {
         foreach(ServiceEndpoint endpoint in endpoints)
         {
            Binding binding = endpoint.Binding;
