...
namespace ServiceModelEx.ServiceBus
{
   public abstract class EventsHost : IServiceBusProperties
   {
      ...
      public void SetServiceBusCertificate(object findValue,StoreLocation location,StoreName storeName,X509FindType findType) 
      {
         ...
         m_Credential.CredentialType = TransportClientCredentialType.X509Certificate;
         m_Credential.Credentials.ClientCertificate.SetCertificate(location,storeName,findType,findValue);
      }
      ...
      public void SetServiceBusPassword(string password)
      {
         ...
         m_Credential = new TransportClientEndpointBehavior();
         m_Credential.CredentialType = TransportClientCredentialType.UserNamePassword;
         m_Credential.Credentials.UserName.UserName = ServiceBusHelper.ExtractSolution(new Uri(BaseAddresses[0]));
         m_Credential.Credentials.UserName.Password = password;
      }
      ...
      public virtual void Subscribe(Type contractType,string operation)
      {
         ...
         if(Hosts[contractType][operation] == null)
         {
            ...
            {
               Hosts[contractType][operation].SetServiceBusPassword(m_ServiceBusPassword);
            }
            if(m_ServiceBusCertFindValue != null)
            {
               Hosts[contractType][operation].SetServiceBusCertificate(m_ServiceBusCertFindValue,m_ServiceBusCertLocation,m_ServiceBusCertStoreName,m_ServiceBusCertFindType);
            }
