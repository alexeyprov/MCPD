    ...
    public class ServiceManagementWrapper
    {
        ...
        public Issuer AddIdentityProvider(string displayName, byte[] fedMetadata)
        {
            try
            {
                ...
                {
                    foreach (byte t in fedMetadata)
                    {
                        postStream.WriteByte(t);
                    }
                }
        ...
        public void AddRelyingPartyWithAsymmetricKey(string relyingPartyName, string realmAddress, string replyAddress, byte[] signingCert = null, string signingCertPassword = "", byte[] encryptionCert = null, string ruleGroupName = "", string[] allowedIdentityProviders = null)
        {
            try
            {
                var client = this.CreateManagementServiceClient();
        ...
        public void AddRelyingPartyWithSymmetricKey(string relyingPartyName, string realmAddress, string replyAddress, byte[] symmetricKey, TokenType tokenType, int tokenLifetime, string ruleGroupName = "", string[] allowedIdentityProviders = null)
        {
            try
            {
                var client = this.CreateManagementServiceClient();
        ...
        public void AddSimpleRuleToRuleGroup(string ruleGroupName, string identityProviderName, string inputClaimType = null, string inputClaimValue = null, string outputClaimType = null, string outputClaimValue = null)
        {
            try
            {
                var rule = new Rule
        ...
        private void AddRuleToRuleGroup(string ruleGroupName, string identityProviderName, Rule rule)
        {
            ...
            rule.IssuerId = identityProvider.Issuer.Id;
            rule.RuleGroup = ruleGroup;
        ...
        public string GetTokenFromAcs()
        {
            ...
            var client = new WebClient
                {
                    BaseAddress = string.Format(CultureInfo.CurrentCulture, "https://{0}.{1}", this.serviceNamespace, Constants.AcsHostName)
                };

            var values = new NameValueCollection
                {
                    { "grant_type", "client_credentials" },
                    { "client_id", this.serviceIdentityUsernameForManagement },
                    { "client_secret", this.serviceIdentityPasswordForManagement },
                    { "scope", client.BaseAddress + "v2/mgmt/service/" }
                };
