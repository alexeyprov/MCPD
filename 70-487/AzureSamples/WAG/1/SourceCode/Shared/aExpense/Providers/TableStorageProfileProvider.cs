...
namespace AExpense.Providers
{
    ...
    public class TableStorageProfileProvider : ProfileProvider
    {
            ...
            try
            {
                foreach (string username in usernames)
                {
                    ...
                    var query = (from profile in queryObj
                                 where profile.PartitionKey == SecUtility.CombineToKey(this.applicationName, usernameForQuery)
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Reliability", "CA2000:Dispose objects before losing scope", Justification = "Dispose is done by the StreamReader object")]
        public override SettingsPropertyValueCollection GetPropertyValues(SettingsContext context, SettingsPropertyCollection collection)
        {
            ...
            try
            {
                ...
                {
                    if (!this.blobProvider.GetBlobContentsWithoutInitialization(profile.ProfileBlobName, reader.BaseStream, out blobProperties) ||
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            this.ApplicationName = Configuration.GetStringValueWithGlobalDefault(
                    config,
                    "applicationName",
                    Configuration.DefaultProviderApplicationNameConfigurationString,
                    Configuration.DefaultProviderApplicationName,
                    false);

            // profile information are stored in the membership table
            this.tableName = Configuration.GetStringValueWithGlobalDefault(
                config,
                "membershipTableName",
                Configuration.DefaultMembershipTableNameConfigurationString,
                Configuration.DefaultMembershipTableName,
                false);

            this.containerName = Configuration.GetStringValueWithGlobalDefault(
                config,
                "containerName",
            ...
            try
            {
                account = CloudConfiguration.GetStorageAccount(Configuration.DefaultStorageConfigurationString);
                ...
                this.tableStorage.RetryPolicy = this.tableRetry;
                this.tableStorage.CreateTableIfNotExist<MembershipRow>(this.tableName);
            ...
            catch (Exception e)
            {
                ...
                if (account != null)
                {
                    ...
                    string tableNameForErrorMessage = this.tableName ?? "no profile table name specified";
                    string containerNameForErrorMessage = this.containerName ?? "no container name specified";
                    Log.Write(
                            EventKind.Error,
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Reliability", "CA2000:Dispose objects before losing scope", Justification = "Dispose is done by the StreamWriter object")]
        public override void SetPropertyValues(SettingsContext context, SettingsPropertyValueCollection collection)
        {
            ...
            try
            {
                using (var writer = new StreamWriter(new MemoryStream()))
                {
                    ...
                    writer.Flush();
                    blobName = this.GetProfileBlobPrefix(username) + Guid.NewGuid().ToString("N");
                    writer.BaseStream.Seek(0, SeekOrigin.Begin);
                    this.blobProvider.UploadStream(blobName, writer.BaseStream);

                    this.CreateOrUpdateUserAndProfile(username, authenticated, DateTime.UtcNow, blobName, (int)writer.BaseStream.Length);
                }
            }
            catch (Exception e)
            {
        ...
        private ProfileInfoCollection GetProfilesForQuery(string usernameToMatch, DateTime inactiveSinceUtc, ProfileAuthenticationOption auth, int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            try
            {
                ...
                for (i = startIndex; i < endIndex && i < users.Count && userMatches; i++)
                {
                    ...
                    infoColl.Add(new ProfileInfo(user.UserName, user.IsAnonymous, user.LastActivityDateUtc.ToLocalTime(), user.ProfileLastUpdatedUtc.ToLocalTime(), user.ProfileSize));
                }
